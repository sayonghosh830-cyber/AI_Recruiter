...
export default function App() {
  ...
  // 1. Initial Load: Disabled to support purely user-uploaded candidates logic
  // fetchCandidates() is only called to synchronize catalog as needed.
  ...
  const runRankingPipeline = async (uploadPayload?: RawCandidateProfile[]) => {
    ...
    setSuccessText(null);
    try {
      const activePayload = uploadPayload || (candidates.length > 0 ? candidates : undefined);
      if (!activePayload || activePayload.length === 0) {
        throw new Error("No candidate profiles loaded. Please upload a compatibly formatted JSON file first.");
      }
      const res = await fetch("/api/rank", {
        ...
        body: JSON.stringify({
          candidates: activePayload || null // If null, the backend uses existing candidates catalog
        }),
      });
              ...
              <div className="space-y-4">
                {rankedRecords.length === 0 ? (
                  <div className="bg-[#151b23] border border-dashed border-[#30363d] rounded-xl p-12 text-center">
                    ...
                    <h3 className="text-white text-base font-semibold mb-1">No Candidate Data Ranked</h3>
                    <p className="text-xs text-[#8b949e] max-w-md mx-auto leading-relaxed mb-4">
                      The candidate prioritisation matrix is empty. Please upload candidate profiles (.json) in the sidebar panel and click "Rank Candidates" to run.
                    </p>
                    {candidates.length > 0 && (
                      <button
                        onClick={() => runRankingPipeline()}
                        disabled={isLoading}
                        className="bg-[#238636] hover:bg-[#2ea043] text-white text-xs font-semibold py-2 px-6 rounded-lg cursor-pointer transition-colors"
                      >
                        Rank Ingested Catalog
                      </button>
                    )}
                  </div>
                ) : (
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left text-xs border-collapse">
                          ...
                          <tbody className="divide-y divide-[#21262d]">
                            {filteredRankedResult.map((row, index) => {
