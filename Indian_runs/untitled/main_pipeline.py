...
def run_pure_python_fallback_pipeline(config_path: str = "config.json") -> None:
    """
    ...
    job_title = job_cfg.get("job_title", "Backend Software Engineer")
    job_description = job_cfg.get("job_description", "")
    required_skills = job_cfg.get("required_skills", [])
    ...
    for idx, cand in enumerate(raw_candidates_list):
        ...
        if final_match_score >= 0.50:
            ...
            suggested_action = f"Schedule immediate hiring reviews. Candidate satisfies {len(matched_target_skills)} core technical requirements with high platform responsiveness metrics."
        elif final_match_score >= 0.35:
            hiring_confidence = "RECOMMENDED"
            suggested_action = f"Progress to screening interviews. Solid alignment with '{current_title}' parameters. Evaluate notice timeline constraints early in discussion."
        elif final_match_score >= 0.25:
            hiring_confidence = "CONSIDER_WITH_CAUTION"
            suggested_action = f"Schedule screening call only if secondary candidates are sparse. Review skill gaps of: {', '.join(missing_target_skills[:2])}."
        else:
            hiring_confidence = "NOT_ALIGNED"
            suggested_action = "Reject profile or place in alternative pipelines."
...
def run_end_to_end_pipeline(config_path: str = "config.json") -> None:
    ...
    if USING_PURE_PYTHON_FALLBACK:
        run_pure_python_fallback_pipeline(config_path)
        return
